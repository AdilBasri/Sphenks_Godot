...
@export var boss_tipi: String = "golem"
...
func saldiri_baslat():
	...
	if arayuz and arayuz.has_method("bilgi_goster"):
		arayuz.bilgi_goster(DilYoneticisi.metin_al("kaya_firlatiyor"), 1.5)
...
func _hp_ayarla():
	...
	if GameManager:
		...
		boss_hp = 2
	print("🪨 STONE BOSS HP: %d (Kalıcı)" % boss_hp)
...
func _mevcut_hitboxlari_yapilandir():
	...
	for hb in hitboxes:
		...
		print("🎯 STONE Sahne hitbox'ı yapılandırıldı: ", hb.name)
...
func mermi_hasari_al(hit_pos: Vector3, hit_dir: Vector3):
	...
	boss_hp -= 1
	print("🔫 STONE BOSS'a vuruş yapıldı! Kalan HP: %d" % boss_hp)
...
func _boss_oldu_mermi():
	...
	print("☠️ STONE BOSS MERMİYLE ÖLDÜRÜLDÜ!")
...
func _fiziksel_temizlik_yap():
	...
	print("☢️ Stone Boss fiziksel nesneleri NUCLEAR olarak temizlendi.")
...
func gercek_saldiri_basa_don():
	...
	print("⏳ Ghost Move bitti (Stone). Boss saldırıya geçiyor!")
