...
# Böylece sen Boss'u enine veya boyuna çekiştirirsen o şekli korur.
...
func _process(delta):
	...
	# ARTIK ANA_SCALE YOK!
	# Senin editörde ayarladığın "baslangic_scale" üzerine ekleme yapıyoruz.
	scale = Vector3(
		baslangic_scale.x + scale_degisimi, 
		baslangic_scale.y - (scale_degisimi * 0.2), 
		baslangic_scale.z + scale_degisimi # Z eksenini de nefese dahil ettim, daha hacimli durur
	)
